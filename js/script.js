$(document).ready(function () {
	...
	$('#signInUpContainer').on('click', function (event) {
		...
		if (event.target == $('#choseLogIn')[0]) {
			...
			$('#choseLogIn').css({background: 'white',color: '#333'}).siblings().css({background: '#333',color: 'white'});
		}
		else if (event.target == $('#choseSignUp')[0]) {
			$('#login').css('display', 'none');
			$('#signup').css('display', 'block');
			$('#choseSignUp').css({background: 'white',color: '#333'}).siblings().css({background: '#333',color: 'white'});
		}
	...
	$('#editProfile').on('click', function(event) {
		...
		$('#profileInfo').find('section').css({background: '#fff', margin: '0', padding: 0});
	});
